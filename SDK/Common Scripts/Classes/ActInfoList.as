...
class ActInfoList {
	...
	public function GetAct(act:Number) : ActInfo
	{
		...
		acti = new ActInfo(slaveData, coreGame);
	...
	public function IsActDefinedByName(str:String) : Boolean
	{
		...
		// search array for an existing act
		for (var ai:String in ActArray) {
			if (ActArray[ai].strNodeName.toLowerCase() == str) return true;
		}
		return false;
	}
	...
	public function Load(lo:Object)
	{
		...
		for (var i:Number = 0; i < len; i++) { 
			if (lo.ActArray[i].Act >= 0) {
				var act:ActInfo = GetAct(lo.ActArray[i].Act);
	...
	public function ShowFlags() : String
	{
		...
		for (var i:Number = 0; i < len; i++) {
			act = GetActByIndex(i);
			say += "<b>Act " + act.Act + "</b>: " + act.strNodeName + "\r  Type: " + act.Type + "\tShown: " + act.bShown + "\r  Counts: " + GetNormalActCount(act.Act) + "," + GetDickgirlActCount(act.Act)+ "," + GetCatgirlActCount(act.Act) + "\r";
		}
		return say;
	}
